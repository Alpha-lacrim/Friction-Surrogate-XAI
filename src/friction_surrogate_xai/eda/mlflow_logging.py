"""MLflow logging for EDA artifacts."""
...
class EDAMLflowLogger:
    """Log EDA outputs into MLflow."""
    ...
    def log_dataset_run(
        ...
        """Log one dataset's EDA outputs into MLflow."""
        if not self.enabled():
            return
        ...
        with mlflow.start_run(run_name=f"eda_{dataset_key}"):
            mlflow.set_tag("dataset", dataset_key)
            for tag_key, tag_value in self.config.get("tags", {}).items():
                mlflow.set_tag(tag_key, tag_value)
            mlflow.log_params(params)
            mlflow.log_metrics(metrics)
            artifact_prefix = self.config.get("artifact_path_prefix", "eda")
            mlflow.log_artifacts(str(artifact_dir), artifact_path=f"{artifact_prefix}/{dataset_key}")
